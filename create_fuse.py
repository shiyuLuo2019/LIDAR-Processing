...
def run(drive_fp, drive_out_dir, lat1, lat2, lon1, lon2, use_bbox, sample_rate, level_of_detail=22, buf_size=0.2, use_buf=False, verbose=True, fmt='npy', remove_dump=False, drive_counter=0):
    ...
    # resize bbox
    epsilon = 10 ** -3
    min_lat, min_lon, _ = np.min(points, axis=0) - epsilon
    max_lat, max_lon, _ = np.max(points, axis=0) + epsilon
    if use_bbox:
...
if __name__ == "__main__":
    ...
    for drive in drives:
        ...
        run(drive, drive_out_dir, lat1, lat2, lon1, lon2, use_bbox, sample_rate, level_of_detail, buf_size, use_buf, True, fmt, remove_dump, drive_counter)
        drive_counter += 1
